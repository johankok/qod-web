...
app.get("/", async function (req, res) {
  ...
  try {
    const quote = await fetch(QOD_API_URL + "/daily").then((r) => r.json());
    var now = new Date();
    ...
    logErr(error);
    res.render("home", { title: "QOD Error", today: "", quote: error, author: "", genre: "" });
  }
});
...
app.get("/random", async function (req, res) {
  ...
  try {
    const quote = await fetch(QOD_API_URL + "/random").then((r) => r.json());
    quote.today = "";
